...
def render_camera(cam_id="camera"):
    """Renders a live camera feed using the browser's native getUserMedia API."""
    camera_html = f"""
    <div id="{cam_id}-wrap" style="
        background: #0a0a0a;
        border-radius: 14px;
        overflow: hidden;
        ...
        margin: 0.5rem auto;
    ">
        <video id="{cam_id}" autoplay playsinline muted style="
            width: 100%;
            display: block;
            border-radius: 12px;
            transform: scaleX(-1);
        "></video>
        <div id="{cam_id}-status" style="
            position: absolute;
            top: 12px; left: 14px;
    ...
    <script>
    (function() {{
        const video = document.getElementById('{cam_id}');
        const status = document.getElementById('{cam_id}-status');
        const errorDiv = document.getElementById('{cam_id}-error');
        navigator.mediaDevices.getUserMedia({{ video: {{ facingMode: 'user', width: {{ ideal: 640 }}, height: {{ ideal: 480 }} }}, audio: false }})
            ...
            .catch(function(err) {{
                ...
                if (err.name === 'NotAllowedError') {{
                    errorDiv.innerHTML = '🚫 Camera access denied.<br><small>Please allow camera permission in your browser and refresh.</small>';
                }} else if (err.name === 'NotFoundError') {{
                    errorDiv.innerHTML = '📷 No camera found.<br><small>Please connect a camera and refresh.</small>';
                }} else {{
                    errorDiv.innerHTML = '❌ Camera error: ' + err.message;
                }}
            }});
    }})();
    </script>
...
# ================================================
# PAGE 3 — MEET  (only this section has camera added)
# ================================================
def render_meet():
    ...
    .cam-label { color: #2dd4bf; font-weight: 700; font-size: 1rem; margin-top: 1.5rem; margin-bottom: 0.4rem; }
    </style>
    """, unsafe_allow_html=True)
    ...
    if "meeting_role" not in st.session_state:
        st.session_state.meeting_role = ""
    ...
    # ── MEETING ROOM PAGE (separate camera page) ──────
    if st.session_state.meet_page == "meeting_room":
        role = st.session_state.get("meeting_role", "participant")
        ...
        with header_left:
            if role == "host":
                st.markdown(f"""
                <div style="background: rgba(15,23,42,0.8); padding: 14px 22px; border-radius: 12px;
                    border: 1px solid rgba(45,212,191,0.3); margin-bottom: 0.8rem;">
                    <span style="color: #2dd4bf; font-weight: 700; font-size: 1.1rem;">🎓 Hosting:</span>
                    <span style="color: #f8fafc; font-size: 1.05rem; margin-left: 8px;">
                        {info.get('topic', 'Classroom Session')}</span>
                    <span style="color: #94a3b8; margin-left: 16px;">| Host: {info.get('host_name', 'Teacher')}
                    | Duration: {info.get('duration', '60 mins')}</span>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown(f"""
                <div style="background: rgba(15,23,42,0.8); padding: 14px 22px; border-radius: 12px;
                    border: 1px solid rgba(45,212,191,0.3); margin-bottom: 0.8rem;">
                    <span style="color: #2dd4bf; font-weight: 700; font-size: 1.1rem;">📹 In Meeting:</span>
                    <span style="color: #f8fafc; font-size: 1.05rem; margin-left: 8px;">
                        {info.get('name', 'Guest')}</span>
                    <span style="color: #94a3b8; margin-left: 16px;">| Code: {info.get('meeting_code', 'N/A')}</span>
                </div>
                """, unsafe_allow_html=True)
        with header_right:
            if st.button("🔴 Leave Meeting", use_container_width=True, key="leave_meeting"):
                st.session_state.meet_page = "home"
                st.session_state.meeting_role = ""
                st.session_state.meeting_info = {}
                st.rerun()
        ...
        render_camera("room-cam")

        # Meeting controls bar
        st.markdown("""
        <div style="display: flex; justify-content: center; gap: 18px; margin-top: 1.2rem;">
            <div style="background: rgba(15,23,42,0.75); border: 1px solid rgba(45,212,191,0.25);
                border-radius: 12px; padding: 12px 28px; text-align: center; color: #e2e8f0;
                font-size: 14px;">🎤 Mic On</div>
            <div style="background: rgba(15,23,42,0.75); border: 1px solid rgba(45,212,191,0.25);
                border-radius: 12px; padding: 12px 28px; text-align: center; color: #e2e8f0;
                font-size: 14px;">📷 Camera On</div>
            <div style="background: rgba(15,23,42,0.75); border: 1px solid rgba(45,212,191,0.25);
                border-radius: 12px; padding: 12px 28px; text-align: center; color: #e2e8f0;
                font-size: 14px;">💬 Chat</div>
            <div style="background: rgba(15,23,42,0.75); border: 1px solid rgba(45,212,191,0.25);
                border-radius: 12px; padding: 12px 28px; text-align: center; color: #e2e8f0;
                font-size: 14px;">👥 Participants</div>
        </div>
        """, unsafe_allow_html=True)

    # ── JOIN PAGE (form only, no camera) ──────────────
    elif st.session_state.meet_page == "join":
        st.markdown('<div class="main-card">', unsafe_allow_html=True)
        st.markdown('<div class="page-title">Join a Meeting</div>', unsafe_allow_html=True)
        st.markdown('<div class="page-sub">Enter your details and click Join Meeting to open the meeting room.</div>', unsafe_allow_html=True)
        ...
        with col1:
            if st.button("Join Meeting", use_container_width=True):
                st.session_state.meeting_role = "participant"
                st.session_state.meeting_info = {"name": name or "Guest", "meeting_code": meeting_code or "N/A"}
                st.session_state.meet_page = "meeting_room"
                st.rerun()
        with col2:
            if st.button("Back to Home", use_container_width=True, key="join_back"):
                go_to("home"); st.rerun()
        if name or meeting_code:
            st.info(f"Ready to join as: {name or 'Guest'} | Code: {meeting_code or 'Not entered'}")
        st.markdown("</div>", unsafe_allow_html=True)

    # ── HOST PAGE (form only, no camera) ──────────────
        ...
        with col1:
            if st.button("Start Meeting", use_container_width=True):
                st.session_state.meeting_role = "host"
                st.session_state.meeting_info = {"host_name": host_name or "Teacher", "topic": topic or "Classroom Session", "duration": duration}
                st.session_state.meet_page = "meeting_room"
                st.rerun()
        with col2:
            if st.button("Back to Home", use_container_width=True, key="host_back"):
                go_to("home"); st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
